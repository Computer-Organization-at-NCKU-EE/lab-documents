...
source operands (or store data), immediates, `rd/rs1/rs2`, `PC/PC+4` (as needed), and **all control bits** required later (e.g., `RegWrite`, `MemRead/Write`, `MemToReg`, ALU-op). Carrying control forward lets a younger instruction act correctly several cycles after decode.

#### Frequency benefit 
By cutting the path, the clock period becomes

$$
T_{\mathrm{clk}} \approx \max(\text{stage delays}) + t_{\mathrm{reg}},
$$

where $t_{\mathrm{reg}}$ is the register overhead (clk-to-Q, setup, clock skew).
Staging **raises the maximum clock frequency** (throughput $\uparrow$) even
though a single instruction’s **latency** increases to
$N_{\text{stages}} \times T_{\mathrm{clk}}$.
