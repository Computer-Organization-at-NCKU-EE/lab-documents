...
By contrast, when discussing ***Micro-architecture***, the term refers to the hardware implementation details of a processor or other hardwares.
...
The *NOP (No-operation)* instruction is actually the `ADDI x0, x0, 0`. The instruction will not change any **architectural visible state**, except for advancing the PC and some other related *performance counters*.

### Control Transfer Instructions

::: {.callout-note title="Question on the encoding of the immediates for JAL and BRANCH instructions"}
TODO
:::
...
For load and store instructions, the most important *parameter* for these instruction is the **width**.
...
| SRLI             | OP-IMM     | `rd = rs1 >> imm[4:0]`                          |
| SRAI             | OP-IMM     | `rd = unsigned(rs1) >> imm[4:0]`                |
| ADD              | OP         | `rd = rs1 + rs2`                                |
| SUB              | OP         | `rd = rs1 - rs2`                                |
...
| XOR              | OP         | `rd = rs1 ^ rs2`                                |
| SRL              | OP         | `rd = rs1 >> rs2[4:0]`                          |
| SRA              | OP         | `rd = unsigned(rs1) >> rs2[4:0]`                |
| OR               | OP         | `rd = rs1 | rs2`                                |
| AND              | OP         | `rd = rs1 & rs2`                                |
