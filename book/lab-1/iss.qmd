...
A key point to note is that updates to the ***architectural state*** occur only during the execution stage. Neither the fetch stage nor the decode stage modifies the architectural state.
...
In the RISC-V ISA --- particularly in the RV32I subset that we aim to implement—the architectural state mainly consists of two components:
...
The reason to define `inst_enum_t` is that it can simplify the implementation of execute stage of the ISS.
...
We will introduce how to implement decode logic with `inst_fields_t` and `inst_enum_t` in @sec-iss-decode.

Furthermore, `inst_fields_t` and `inst_enum_t` also help us to implement execution stage in a more easy way, which will be introduced in @sec-iss-exe.
...
You can view the function `MemoryMap_generic_load()` as a normal function to access the memory at this point, and the concepts about memory map and its implementaton will be metioned at @sec-memory-map.
...
As what we have metioned in @sec-iss-predefine, we can utilize `inst_fields_t` to decode instructions, and produces `inst_enum_t`.
...
After fetching and decoding a instruction, finally, we can now execute the instruction and **update architectural visible states**, and then update the **program counter**.
...
The reason why we separate PC updating and other execution logics into two functions `Core_update_pc()` and `Core_execute()` respectively is to make the implementation simpler.
...
In order to manipulate a generic memory device, the processor must know the folowing two things:

1. Memory boundary of the device
2. A pointer to the device (more specifically, a `AbstractMem *` to the device)

As the result, we define `addr_pair_t` at the beginning, and then define `mmap_unit_t` which contains the two things we metion above.
...
## How To Build The Lab 1 Code Template
...
For example, if my folder is called `lab-1`, then I can build the code template as following:
