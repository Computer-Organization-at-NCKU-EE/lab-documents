...
In order to manipulate a generic memory device, the processor must know the folowing two pices of information:

1. Memory boundary of the device
2. A pointer to the device (more specifically, a `AbstractMem *` to the device)

As the result, we define `addr_pair_t` at the beginning, and then define `mmap_unit_t` which contains the two pices we metion above.
...
3. Call to the actual implementation of load function the device which interits from `AbstractMem` via `AbstractMem_load()` function.
