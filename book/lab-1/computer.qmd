...
![](images/sw-hw-intf.png){width=75% fig-align="center"}
...
In general, computers are often referred to as the hardware in the figure above. A computer is a kind of hardware implementation which can recognize one or more kinds of ISA. With the power of ISA, the computer (hardware) can execute different programs and complete specific tasks.

![This figure is from @comporg-book](images/code-inst.png){width=75% fig-align="center"}
...
Please note that the binaray represenation of each instruction are also defined by ISA.

In summary, an ISA can be regarded as a formal specification that defines:

1. The instruction set and its encoding formats
2. The execution semantics of each instruction
3. Additional rules governing program behavior (e.g., traps, exceptions, memory model)
...
These FSMs are easy to draw on paper and are often used to describe control logic.
...
![Harvard Architecture, [Source](https://en.wikipedia.org/wiki/Harvard_architecture)](images/harvard-arch.png){width=75% fig-align="center"}
...
Such as the microprocessor above, it has separated instruction and data memories, which is *Harvard Architecture*.
...
![A Simple Computer System](images/simple-sys.png){width=75% fig-align="center"}
