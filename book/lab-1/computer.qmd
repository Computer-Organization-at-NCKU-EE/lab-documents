...
So, what is a computer? There may be many different answers depending on who you ask.
I would say that a computer is a kind of machine that [**can be programmed to perform tasks automatically**]{.mark}.
...
In contrast, modern computers can carry out a wide variety of general-purpose tasks.
...
From another perspective, a computer can be viewed as a large and complex finite-state machine (FSM). Under this view, the two most important questions are:

1. What is the current states of the computer?
2. How does the state transition occur within the computer?

By definition, the states of an FSM are stored in ***memory elements***.
In a computer, these memory elements are the processor’s registers and memory devices (units).

![The FSM point of view](images/computer-fsm.png){width=75% fig-align="center"}

The ***state transition function*** for a processor implements specific ISA is the **rules** defined in the ISA specification.
...
These refer to the ***traps*** in the RISC-V ISA, such as ***exceptions*** or ***interrupts***.
While traps are crucial for real processors, we will not cover them in this lab.
...
Such as the microprocessor above, its has separated instruction memory and data memory, which is *Harvard Architecture*.
...
At this point, we already understand what a computer is and how it works internally.
However, a big question remains: **how do the different components inside a computer communicate with each other?**
To explore this, let’s consider a very simple embedded system, shown below.

![A Simple Computer System](images/simple-sys.png){width=50% fig-align="center"}

How can the CPU control the LED light? One straightforward approach is to treat the LED as a type of [***memory-mapped device***]{.mark}.
...
For the processor, it can turn on the LED light through the following instruction:
...
Now suppose we want the processor to automatically turn off the LED whenever it is on. A simple program might look like this:
